...
- the quality gate is repo-aware: prepare npm dependencies when needed, run `make check` when a root `Makefile` declares `check`, otherwise run `npm run check` when a root `package.json` declares it, otherwise treat the repo as having no configured gate
